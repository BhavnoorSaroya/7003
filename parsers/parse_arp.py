...
def parse_arp_header(hex_data):
    ...
    sender_IP = '.'.join(str(int((hex_data[i:i+2] for i in range(28, 36, 2)))))
    target_mac = ':'.join(hex_data[i:i+2] for i in range(36, 48, 2))
    target_IP = '.'.join(hex_data[i:i+2] for i in range(48, 56, 2))
    ...
    print(f"  {'Hardware Size:':<25} {hex_data[4:8]:<20} | {hardware_size}")
    print(f"  {'Protocol Size:':<25} {hex_data[4:8]:<20} | {protocol_size}")
    print(f"  {'Operation:':<25} {hex_data[4:8]:<20} | {operation}")
    print(f"  {'Sender MAC':<25} {hex_data[4:8]:<20} | {sender_mac}")
    print(f"  {'Sender IP':<25} {hex_data[4:8]:<20} | {sender_IP}")
    print(f"  {'Target MAC:':<25} {hex_data[4:8]:<20} | {target_mac}")
    print(f"  {'Target IP:':<25} {hex_data[4:8]:<20} | {target_IP}")
